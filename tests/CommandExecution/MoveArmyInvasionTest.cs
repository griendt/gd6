...
public class MoveArmyInvasionTest : BaseTest
{
    ...
    public void ItAppliesFortressRulesForInvasion(int numSent, int numExpectedInTarget, int numExpectedUnprocessed, bool isFortressRuined)
    {
        ...
        Assert.Multiple(() =>
        {
            Assert.That(T(2).Units.Armies, Is.EqualTo(numExpectedInTarget));
            Assert.That(T(2).IsNeutral, Is.EqualTo(numExpectedInTarget == 0));
            Assert.That(T(2).Constructs, Does.Contain(isFortressRuined ? Construct.Ruin : Construct.Fortress));
        });
    }
    [TestCase(1, 2, 0)]
    [TestCase(2, 2, 0)]
    ...
    public void ItDestroysABivouac(int numSent, int numExpectedInTarget, int numExpectedUnprocessed)
    {
        ...
        Assert.Multiple(() =>
        {
            ...
            Assert.That(T(2).Constructs, numExpectedInTarget == 0 
